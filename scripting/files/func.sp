...
{
	int iIndex = g_arPatchNum.FindValue(iDefIndex);
	if(iIndex == -1)
	{
		return -1;
	}
...
{
	if(g_arPatchNum.Length < iPatchNum)
	{
		return -1;
	}

	return g_arPatchNum.Get(iPatchNum);
}
...
public bool GetPatchDisplayNameByDefIndex(int iDefIndex, char[] szDisplayName, int iLen)
{
	...
	g_smPatchInfo.GetArray(szDefIndex, PatchInfo, sizeof(ePatchInfo));
...
public int GetCratesNumByDefIndex(int iDefIndex)
{
...
public int GetCratesDefIndexByCratesNum(int iCratesNum)
{
	if(g_arCratesNum.Length < iCratesNum)
	{
		return -1;
	}

	return g_arCratesNum.Get(iCratesNum);
}

public bool GetCratesDisplayNameByDefIndex(int iDefIndex, char[] szDisplayName, int iLen)
{
	char szDefIndex[12];
	IntToString(iDefIndex, szDefIndex, sizeof(szDefIndex));

	eCratesInfo CratesInfo;
	g_smCratesInfo.GetArray(szDefIndex, CratesInfo, sizeof(eCratesInfo));

	strcopy(szDisplayName, iLen, CratesInfo.DisplayName);
	return true;
}

public bool GetCratesDisplayNameByCratesNum(int iCratesNum, char[] szDisplayName, int iLen)
{
	int iDefIndex = GetCratesDefIndexByCratesNum(iCratesNum);
...
public bool GetCratesViewModelByCratesNum(int iDefIndex, char[] szViewModel, int iLen)
{
	char szDefIndex[12];
	IntToString(iDefIndex, szDefIndex, sizeof(szDefIndex));
	...
	strcopy(szViewModel, iLen, CratesInfo.ViewModel);
	return true;
}

public bool GetCratesViewModelByDefIndex(int iCratesNum, char[] szViewModel, int iLen)
{
	int iDefIndex = GetCratesDefIndexByCratesNum(iCratesNum);
...
{
	int iIndex = g_arSprayNum.FindValue(iDefIndex);
	if(iIndex == -1)
	{
		return -1;
	}
...
{
	if(g_arSprayNum.Length < iSprayNum)
	{
		return -1;
	}

	return g_arSprayNum.Get(iSprayNum);
}
...
public bool GetSprayDisplayNameByDefIndex(int iDefIndex, char[] szDisplayName, int iLen)
{
	...
	g_smSprayInfo.GetArray(szDefIndex, SprayInfo, sizeof(eSprayInfo));
...
public bool GetSprayMaterialBySprayNum(int iDefIndex, char[] szMaterial, int iLen)
{
	char szDefIndex[12];
	IntToString(iDefIndex, szDefIndex, sizeof(szDefIndex));
	...
	strcopy(szMaterial, iLen, SprayInfo.Material);
	return true;
}

public bool GetSprayMaterialByDefIndex(int iSprayNum, char[] szMaterial, int iLen)
{
	int iDefIndex = GetSprayDefIndexBySprayNum(iSprayNum);
...
public bool TransData(char[] text, int maxLength, StringMap TransCN, StringMap TransEN)
{
	if (!TransCN.GetString(text, text, maxLength))
	{
		if (!TransEN.GetString(text, text, maxLength))
			return false;
	}
	return true;
}

public void StringToLow(char[] str)
{
